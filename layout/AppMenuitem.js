...
const AppMenuitem = (props) => {
    ...
    const iconMenu = typeof item.icon === 'string' ? <i className={classNames('layout-menuitem-icon', item.icon)}></i> : <span className="layout-menuitem-icon">{item.icon}</span>;

    return (
        <li className={classNames({ 'layout-root-menuitem': props.root, 'active-menuitem': active })}>
            {props.root && item.visible !== false && !!item.label && <div className="layout-menuitem-root-text">{item.label}</div>}
