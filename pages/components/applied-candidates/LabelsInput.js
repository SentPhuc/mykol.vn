...
const LabelsInput = (props, { labels }) => {
    ...
    const { row } = props;
    const [selectedLabels, setSelectedLabels] = useState(row?.metaData?.labels?.map((x) => x.name || [])); //selected labels
    const [masterLabels, setMasterLabels] = useState(labels?.map((x) => x.name || [])); //master labels
    const autoCompleteRef = useRef(null);
    const [isShow, setIsShow] = useState(true);

    async function updateLabelsFilter(id) {
        const newLabels = await service.getAllLabels(id);
        if (newLabels.data.code === 'success') {
            ...
            setMasterLabels(data.map((x) => x.name || []));
        } else {
            setMasterLabels([]);
        }
    }
